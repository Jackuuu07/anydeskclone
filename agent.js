...
const KEY_MAP = {
  ...
  " ": Key.Space, CapsLock: Key.CapsLock,
...
// ── Socket ────────────────────────────────────────────────────
const socket = io("https://anydeskclone.onrender.com", {
  reconnection: true,
...
socket.on("connect", () => {
  ...
  if (!currentCode) currentCode = generateCode();
...
socket.on("register-success", ({ code }) => log("📋", `Registered | Code: ${code}`));
...
socket.on("viewer-joined", ({ viewerId }) => log("👀", `Viewer connected: ${viewerId}`));
...
socket.on("control-event", async (event) => {
  ...
  try {
    switch (event.type) {
      ...
      case "click":
        if (event.x !== undefined) await mouse.move(straightTo(new Point(event.x, event.y)));
      ...
      case "key_tap": {
        ...
        if (!key) { log("⚠️ ", `Unmapped key: "${event.key}"`); break; }
      ...
      case "mouse_down":
        await mouse.pressButton(event.button === 2 ? Button.RIGHT : Button.LEFT);
        break;

      case "mouse_up":
        await mouse.releaseButton(event.button === 2 ? Button.RIGHT : Button.LEFT);
        break;
