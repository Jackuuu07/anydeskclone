...
async function createPeerConnection() {
  ...
  peerConnection.ontrack = (e) => {
    ...
    attachControlListeners(); // ✅ move here
  };
...
createBtn.addEventListener("click", () => {
  createBtn.disabled = true; // 🔥 prevent spam
...
shareScreenBtn.addEventListener("click", async () => {
  if (!isHost || !currentCode) return; // 🔥 fix
...
socket.on("code-generated", ({ code }) => {
  currentCode = code;
  showCode(code); // 🔥 important
  setStatus("✅ Code generated. Click Share Screen.", "connected");
  createBtn.disabled = false; // 🔥 fix
});
